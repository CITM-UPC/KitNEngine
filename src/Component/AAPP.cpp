...
// Función que dibuja el AABB de un GameObject utilizando OpenGL moderno
void CalculateAABB(const std::shared_ptr<GameObject>& gameObject)
{
    ...
    // Obtener los vértices de la malla desde mainData
    const std::vector<GLfloat>& vertices = meshRenderer->mainData;

    if (vertices.empty()) {
        return; // No hay vértices
    }

    // Obtener la matriz de transformación del modelo
    glm::mat4 transformMatrix = gameObject->GetTransform()->GetWorldMatrix();

    // Inicializar los límites mínimos y máximos
    glm::vec3 minBounds(FLT_MAX);
    glm::vec3 maxBounds(-FLT_MAX);

    // Iterar sobre los vértices y calcular los límites
    for (size_t i = 0; i < vertices.size(); i += 5) { // mainData almacena posición(3), UV(2)
        glm::vec3 vertex(vertices[i], vertices[i + 1], vertices[i + 2]);

        // Transformar el vértice con la matriz de transformación
        glm::vec4 transformedVertex = transformMatrix * glm::vec4(vertex, 1.0f);

        // Actualizar los límites mínimos y máximos
        minBounds = glm::min(minBounds, glm::vec3(transformedVertex));
        maxBounds = glm::max(maxBounds, glm::vec3(transformedVertex));
    }

    // Dibujar el AABB automáticamente
...
void DrawAABB(const glm::vec3& minBounds, const glm::vec3& maxBounds)
{
    ...
    // Indices para dibujar líneas entre las esquinas
    GLuint indices[] = {
